...
static int	ft_len_str(const char *str, int index)
{
	int	len;
...
static char	**ft_mem_str(int counter_words, const char *str)
{
	char	**ptr;
	char	*string;
	int		index_char;
...
char	**ft_split_av(const char *str)
{
	...
	if (!str)
		return (NULL);
	count = 0;
	counter_words = ft_counter_words(str);
	ptr = ft_mem_str(counter_words, str);
	if (!ptr)
		return (NULL);
	while (count < counter_words - 1)
	{
